...
- (void)start {
    ...
    //    or the embedder. ESCoreDataStack.shared creates the
    ...
    // Automatic deduplication — the archive's integrity guard. Now that only the
    // elected host reaches this point, exactly one writer runs it (before this
    // change a lost-race process ran it redundantly). Idempotent regardless.
    ...
    // after -start, the stdio analogue of the HTTP app's
    // applicationDidFinishLaunching. Kept out of -start so a relay (which returns
    // above, engine never loaded) can't trigger it; only the elected host, which
    // loaded the engine, backfills. Newly stored memories still get their vector
    // at store time regardless.
}
...
- (nullable NSDictionary *)handleRequest:(NSDictionary *)rpc {
    ...
    // Relay to the shared engine when connected. sendRequest: returns nil for
    // notifications (no id) and for a dropped connection; both are the correct
    // "no response" outcome for the stdio writer.
    if (self.remote) return [self.remote sendRequest:rpc];

    // Our own session's persona.
    return [self handleRequest:rpc scope:self.scope];
}
...
- (void)flushAndSave {
    ...
    // closes peer connections and unlinks the socket file, so the next session
    // elects against a clean path instead of probing our corpse (stale-socket
    // recovery covers a crash; a clean exit shouldn't need it). Idempotent —
    // this runs from both the stdio drain and applicationWillTerminate.
    [[MCPUnixSocketServer sharedInstance] stop];

    // viewContext is main-queue bound; ExecuteOnMainThread makes this safe
    // both from the drain (write queue) and from applicationWillTerminate
    // (already on main). -saveContext is a no-op when nothing changed, so
    // calling this twice at shutdown is fine.
    ExecuteOnMainThread(^id {
